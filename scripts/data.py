...
def run():
    ...
    grocery_cat = Category.objects.get_or_create(
        name="grocery", slug="grocery-1", description="grocery items"
    )[0]
...
def upload_carrefour_data(grocery_cat):
    CSV_DATA_FILES = [
        "products_beverages-egyptian.csv",
        "products_dairy-products.csv",
        "products_delicatessen-egyptian.csv",
        "products_frozen-egyptian.csv",
        "products_fruits-veg-egyptian.csv",
        "products_grocery-egyption.csv",
    ]
    for file_name in CSV_DATA_FILES:
        with open(file_name, "r") as file:
            ...
            for row in csvreader:
                try:
                    product_name, brand_name, price, image_url, country = (
                        row[0],
                        row[1],
                        row[2],
                        row[3],
                    ...
                    brand = Brand.objects.get_or_create(
                        name=brand_name, slug=brand_name + "-1", country="EG"
                    )
                    ...
                    image = ProductImage.objects.get_or_create(
                        image_url=image_url, product=product[0], alt=product_name
                    )
                except:
                    continue
        ...
        for row in csvreader:
            try:
                ...
                if price == "":
                    price = "10"
                brand = Brand.objects.get_or_create(
                    name=brand_name, slug=brand_name + "-1", country="CN"
                )
    ...
    with open("amazon_india.csv", "r") as file:
        ...
        for row in csvreader:
            try:
                ...
                brand = Brand.objects.get_or_create(
                    name=brand_name, slug=brand_name + "-1", country=country
                )
