...
from userprofile.form import (
    ShippingAddressForm,
    VisitorShippingAddressForm,
    EditShippingAddress,
)
...
def checkout_view(request):
    ...
    if (
        ...
        and request.get_signed_cookie("checkout", None) is None
    ) or is_none:
        context = {"checkout_line": None}
...
def create_shipping_address(request):
    ...
    if is_auth:
        ...
        form = VisitorShippingAddressForm(
            request.POST or None, instance=checkout.shipping
        )
    context = {"form": form}
...
def edit_shipping_address(request):
    ...
    if user.is_authenticated and request.method == "POST":
        ...
        if form.is_valid():
            checkout = get_checkout(request)
            checkout.shipping = Addresses.objects.get(
                id=int(request.POST.get("addresses"))
            )
            checkout.save()
            return HttpResponseRedirect(reverse("checkout:create_order_view"))
