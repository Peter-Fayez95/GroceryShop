...
class ListProduct(ListView):
    ...
    def get_queryset(self):
        ...
        qs = (
            super(ListProduct, self)
            .get_queryset()
            .filter(category__in=categories, exp_date__gt=datetime.today())
            .order_by("name")
        )
...
class ListProductBrand(ListView):
    ...
    def get_queryset(self):
        ...
        qs = (
            super(ListProductBrand, self)
            .get_queryset()
            .filter(brand__exact=brand, exp_date__gt=datetime.today())
            .order_by("name")
        )
...
def get_or_create_checkout(request, checkout_queryset=Checkout.objects.all()):
    ...
    if user.is_authenticated:
        ...
        return checkout_queryset.get_or_create(user=user, email=user.email)[0]

    token = request.get_signed_cookie("checkout", default=None)
    return checkout_queryset.filter(token=token, user=None).get_or_create(user=None)[0]
