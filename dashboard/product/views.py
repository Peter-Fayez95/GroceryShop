...
class ListProduct(ListView):
    ...
    def get_queryset(self):
        queryset = super().get_queryset().filter(exp_date__gt=datetime.today())
        ...
        if (min_price != "" and min_price is not None) or (
            max_price != "" and max_price is not None
        ):
...
class ProductImageList(ListView):
    ...
    def get_queryset(self):
        return (
            super(ProductImageList, self)
            .get_queryset()
            .filter(product__id=self.kwargs.get("pk"))
        )
        ...
        return JsonResponse(
            {
                ...
                "url": obj.image_url,
                "delete_url": reverse(
                    "dashboard:product_dashboard:delete_image", kwargs={"pk": obj.pk}
                ),
            },
            status=200,
        )
...
def delete_image(request, pk):
    ...
    if product_image:
        ...
        return JsonResponse(
            {"message": "the image has been deleted successfully"}, status=200
        )

    return JsonResponse(
        {"message": "this product image instance does not exist"}, status=400
    )
...
def delete_product(request, pk):
    ...
    if product:
        product.delete()
        return JsonResponse(
            {"message": "the product has been deleted successfully"}, status=200
        )
