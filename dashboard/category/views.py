...
def category_detail(request, pk):
    ...
    context = {"root": root, "path": path, "categories": categories}
...
def delete_category(request, pk):
    ...
    if category:
        category.delete()
        return JsonResponse(
            {"message": "the category has been deleted successfully"}, status=200
        )

    return JsonResponse(
        {"message": "this category instance does not exist"}, status=400
    )
